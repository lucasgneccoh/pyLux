...
if __name__ == "__main__":
  ...
  # Load map
  world = pyRisk.World(prefs['map_path'])
      ...
      if board.activePlayer.human:
        ...
        if board.gamePhase == 'startTurn' and len(board.activePlayer.cards)>=5:
          ...
          if not card_set is None:
            ...
            if console_debug: print("GUI:Cash not possible")
          ...
          if e.type == pygame.QUIT:                    
            ...
            if e.key == pygame.K_ESCAPE:
              ...
              if board.activePlayer.income > 0:
                ...
                if console_debug: print('GUI:Still in initial phases')
              ...
              # Can only do this afterplacing armies
              if board.activePlayer.income > 0:
                if console_debug: print('GUI:Can only end turn after placing armies')
            ...
            if not c_name is None:
              ...
              if c_name == 'Fortify':
                ...
                if board.gamePhase != "startTurn":
                  if console_debug: print("GUI:Cash not possible: Only starting the turn")
            ...
            if console_debug: print(f"GUI:Clicked on {c.id}")
            ...
            if board.gamePhase == 'initialPick':
              if c.owner == -1:
                ...
                if console_debug: print("GUI:Territory already has an owner")
            
  
            if board.gamePhase == 'initialFortify':
              if c.owner == board.activePlayer.code:
                if console_debug: print(f'GUI:Human initial fortifiy: {c.id}')
                numberOfArmies = 1
            ...
            if board.gamePhase == 'startTurn':                              
              ...
              if c.owner == board.activePlayer.code:
                if console_debug: print(f'GUI:Human added armies: {c.id}') 
                numberOfArmies = 1
                if bool_move_all: numberOfArmies = 0
                if bool_move_5: numberOfArmies = 5
            ...
            if board.gamePhase == 'attack':            
              if c.owner == board.activePlayer.code:            
                ...
                if not source is None and c in board.world.getCountriesToAttack(source.code):
                  # Perform attack
                  target = c
              ...
              if c.owner == board.activePlayer.code and source is None:            
                ...
                if not source is None:
                  if c == source:
                    ...
                    draw_lines(coords, board)
                    action_msg = None
                  elif c in board.world.successors(source.code):
                    # Fortify
                    target = c
